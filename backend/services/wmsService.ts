import WMS, { IWMS } from '../models/WMS.js';
...
interface WMSResponse {
    ...
    wms_status?: string;
    customer?: IWMS['customer'];
}

type WMSStatus = 'pending' | 'processing' | 'picking' | 'packing' | 'shipped' | 'delivered' | 'cancelled' | 'error';
...
class WMSService {
    // Connect to WMS
    async connect(): Promise<WMSResponse> {
        try {
    ...
    private formatCustomerData(customerData: CustomerData): IWMS['customer'] {
        let tags: string[] = [];
    ...
    // Sync order with WMS
    async syncOrder(orderId: string, orderData: OrderData): Promise<WMSResponse> {
        try {
    ...
    async getOrderStatus(orderId: string): Promise<WMSResponse> {
        try {
    ...
    async updateCustomer(orderId: string, customerData: CustomerData): Promise<WMSResponse> {
        try {
            const wmsRecord = await WMS.findOne({ orderId });
    ...
    // Update inventory
    async updateInventory(sku: string, quantity: number): Promise<WMSResponse> {
        try {
    ...
    // Log WMS error
    async logError(orderId: string, error: Error): Promise<void> {
        try {
            const wmsRecord = await WMS.findOne({ orderId });
